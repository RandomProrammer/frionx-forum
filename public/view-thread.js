...
/*
reply html 
    <div class="forum-content-box miniforum">
      <label class="forum-form-header">Test name said</label>
      <textarea id="reply-content" class="forum-input-textarea reply-box" placeholder="" readonly>BRO THIS IS DUMB!</textarea>
    </div>
*/

$.get(`/api/v1/get-all-replies/${threadId}`).then(e=>{
  e.replies.forEach(function(reply){
      $.get(`/api/v1/get-username/${reply.author}`).then(e => {
        let divForum = document.createElement('div');
        divForum.setAttribute('class', 'forum-content-box miniforum');
